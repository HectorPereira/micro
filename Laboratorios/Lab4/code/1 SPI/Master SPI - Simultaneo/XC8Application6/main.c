...
//	uart_print("Bienvenido; Pulse A para comenzar\n\r");
	char str_led[10];
	uint8_t escribirLCD = 0;
	while(1)
	{
	...
	//	_delay_ms(300);
		...
		Add_to_string(str_led_adc, lectura_led);
		
		// Convertir valores numéricos a texto (usando tu función)
		Add_to_string(str_temp, Temp);
		Add_to_string(str_hum, Hum);
	//	Add_to_string(str_led, lectura_led);
		//Add_to_string(str_dist, Distancia_cm);
		Add_to_string(str_grados, grados);

		uart_print("\r\n grados: ");
		uart_print(str_grados);
		uart_print(" LED; ");
		uart_print(str_led);
		uart_print(" Lectura adc;");
		uart_print(str_led_adc);
		uart_print(" Lectura distancia;");
		uart_print(str_dist);
		uart_print(" Lectura Temperatura; ");
		uart_print(str_temp);
