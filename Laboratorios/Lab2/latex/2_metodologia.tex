...
\subsubsection{\textbf{Plotter}}
    \paragraph*{\textbf{Movimiento del plotter}}
    El plotter está controlado por un PLC, el cual se encarga de enviar las señales de control a los motores. A su vez, el microcontrolador ATmega328P comunica al PLC las instrucciones necesarias para definir el comportamiento de dichos actuadores.
    Según sea el movimiento de los ejes accionados por dichos motores, se podrá trazar o dibujar 
    una figura dependiendo del programa que se ejecute. 

    \paragraph*{\textbf{Comunicacion via USART}}
    Mediante USART se transmite al plc el estado de las salidas, enviando una serie de datos para trazar un dibujo. En el trazado de figuras mas complejas, se reperesenta mediante pixeles individuales y en las formas geometricas simples(Triangulo y Cruz) salidas constantes calculadas por tiempo, para dibujas la linea correspondiente.
    Para el dibujo del circulo hubieron dos planteamientos dstintos, uno mediante el empleo de formulas trigonometricas y otro mediante la toma de cada bit del circulo individual.
    
    \paragraph*{\textbf{Python para crear datos}}
    Se implemento un codigo en pyhton para poder utilizar imagenes y mediante el uso de sus librerias seleccionar el contorno de la figura e ir trazando su movimiento bit a bit, escribiendolo en un txt que posteriormente se almacenaria en la LUT del atmega328p.

    \paragraph*{\textbf{Dibujo del circulo con seno y coseno}}
    Mediante el uso de librerías matemáticas en lenguaje C es posible calcular las coordenadas exactas del movimiento en los ejes \textit{X} e \textit{Y} para un radio determinado \( r \). A partir de las funciones trigonométricas \textit{seno} y \textit{coseno}, se obtienen los desplazamientos correspondientes a cada ángulo dentro del primer cuadrante, es decir, entre \( 0^\circ \) y \( 90^\circ \).  
    Aplicado al plotter, este procedimiento permite determinar con precisión cuánto debe desplazarse el actuador en cada eje, generando así —mediante incrementos angulares muy pequeños— la trayectoria que describe un cuarto de circunferencia. 
    Repitiendo el proceso 4 veces, cambiando el movimiento en X e Y, entre las combinaciones de Up, Down, Right y Left, se traza un circulo solo utilizando escalones.
    \vspace{1em}
...
\subsubsection{\textbf{Colores}}
    ...
    \paragraph*{\textbf{Descomposición de colores}}---
    ...
    donde $(R_m, G_m, B_m)$ representan los valores medidos por el sensor y $(R_i, G_i, B_i)$ corresponden a los valores calibrados de cada color de la hoja de referencia. El color identificado será aquel cuya distancia $D$ sea mínima (véase anexo~\ref{anexo:reconocimiento_colores}).

    \paragraph*{\textbf{Servomotor y tira LED}}---
    El servomotor se controla mediante el \textit{Timer1} configurado en modo \textit{Fast PWM} a 50\,Hz, generando pulsos de entre 1 y 2\,ms para posicionar el eje según el color identificado. Cada ángulo se calcula a partir del valor del registro \texttt{OCR1A}, que define el tiempo en alto del ciclo PWM (véase anexo~\ref{anexo:servomotor}).

    \vspace{1em}

    La visualización del color se realiza con una tira de LED WS2812, donde cada diodo recibe datos digitales codificados en tres bytes (RGB). Para cumplir con los tiempos estrictos del protocolo, se emplearon instrucciones \texttt{asm volatile} que garantizan una sincronización precisa en la transmisión de pulsos. Así, al identificar un color, el sistema replica su tonalidad en la tira LED y mueve el servomotor al ángulo correspondiente, integrando una respuesta visual y mecánica simultánea (véase anexo~\ref{anexo:tira_led}).

    \vspace{1em}

    \paragraph*{\textbf{USART}}---
   A través de USART se comunican los valores del sensor y el color detectado hacia una interfaz serial. Se utiliza un buffer circular para transmisión continua sin bloqueo, y la función auxiliar \texttt{UTOA} convierte los datos numéricos del ADC en texto ASCII antes de su envío. De esta forma, el sistema comunica en tiempo real los valores $(R, G, B)$ medidos, el color identificado y el ángulo del servomotor, facilitando el monitoreo y la calibración del sistema (véase anexo~\ref{anexo:USART}).
...
\subsubsection{\textbf{Piano}}
    ...
    En el microcontrolador ATmega328P, esta señal se genera mediante el uso de los temporizadores internos (\textit{timers}), configurados para producir una onda cuadrada en un pin de salida. Al modificar la frecuencia de conmutación, es posible variar el tono percibido, ya que la frecuencia del sonido determina su altura musical. De esta manera, cada nota puede asociarse a una frecuencia específica según la escala. Por ejemplo, la nota \textit{La4} corresponde a una frecuencia de 440\,Hz, mientras que \textit{Do4} equivale aproximadamente a 262\,Hz (véase anexo~\ref{anexo:notas_musicales}).


    \vspace{1em}

    \paragraph*{\textbf{Generación de frecuencias}}---
    Para reproducir una nota musical, el microcontrolador debe generar una señal cuadrada con una frecuencia específica, determinada por el valor de comparación del temporizador. En el ATmega328P, dicha frecuencia depende de la relación entre la frecuencia del reloj del sistema (\texttt{F\_CPU}), el valor de comparación \texttt{OCRx}, y el prescaler aplicado al temporizador. Este último actúa como un divisor de frecuencia, reduciendo la velocidad a la que el contador incrementa.
    
    \vspace{1em}

    Elegir el prescaler adecuado resulta fundamental para asegurar que el valor de \texttt{OCRx} se mantenga dentro del rango permitido por el temporizador (por ejemplo, 8 bits en el Timer0). Si la frecuencia deseada es alta, se requiere un prescaler pequeño para mantener precisión; si es baja, un prescaler grande evita desbordamientos y permite generar tonos audibles. 

    \vspace{1em}

    En la práctica, el programa selecciona dinámicamente el prescaler que produce un valor de \texttt{OCR0A} válido para la nota solicitada (véase anexo~\ref{anexo:piano_prescaler}). De esta forma, es posible generar un amplio rango de frecuencias musicales con un solo temporizador, manteniendo una relación estable entre tono y duración.
...
\subsubsection{\textbf{Cerradura}}
    ...
    \begin{figure}[H]
        ...
        \caption{Diagrama de flujo de programa de cerradura. Fuente: elaboración propia}
        \label{fig:lcd_ui}
    \end{figure}
